...
**丢失更新**（实测热行碰撞下每千次丢十余次）。修复：

- `external_lock()` 收到写锁时置 `write_stmt` 标志（仅句柄内标志，
  **无任何表级锁**）；
- 写语句期间，每次读行（`index_read*` / `rnd_next` / `rnd_pos`）即对
  命中的行槽取 `wlock` 并跨调用持有；
- `update_row()` / `delete_row()` 在锁内执行随后释放；
- 效果：写者之间的整个读-算-写原子化（等价行锁，逐行释放），
  **纯 SELECT 完全不进锁路径**，无锁读保持。
...
- 高碰撞下 InnoDB 的差异来自行锁排队；FASTMEM 无表锁 + 读者无锁，
  写者行级自旋 → 耗时几乎与碰撞强度无关；
- 修复过的读-算-写路径保证并发 UPDATE 不丢更新，SUM 增量与语句数
  精确相等；
