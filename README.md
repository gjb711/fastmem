...
把 `storage/fastmem/` 目录放进 MariaDB 源码树（本仓库的
`D:\maria-src` 即可），然后正常 configure：
...
> 注意：本源码树来自 `mariadb-12.1.1` 发布包，git 子模块
> （`extra/wolfssl/wolfssl` 需 **v5.7.6-stable**、`libmariadb`、`wsrep-lib` 等）
> 未包含在内。构建全服务器前需补齐并对齐版本：
...
> - pcre2 由 cmake ExternalProject 联网下载（需可访问 github.com，
...
- `TRUNCATE` 清空并复用内存；
- 无持久化：服务器重启数据清空；
- 不支持 `ALTER ... ENGINE=FASTMEM` 之外需要重建且含 BTREE 索引的
  DDL（显式 BTREE 会报错）。
...
## 8. 实测基准（mariadb-12.1.1 / Windows x64 / VS2022 真机）

> 环境：`mariadb-12.1.1-MariaDB`，端口 3309，本地连接；表为 7000 行
> 行情 K 线数据（主键 varchar(50) + 2 个普通 HASH 索引）；基准脚本在
> `bench/` 目录。

| 场景 | FASTMEM | 对照 | 说明 |
|------|---------|------|------|
| 单连接 UPDATE/INSERT/SELECT 基线 | 1.00x | MEMORY 基准 | UPDATE 0.92x / SELECT 1.06x / INSERT 1.00x（略快或持平） |
| **并发热行碰撞 8W+4R×300** | **1.24 s** | InnoDB 1.66 s | 2400 次碰撞更新命中 100 热行 + 1200 次并发读，双方 **delta=+2400 精确零丢失** |
| 纯 UPDATE 负载 8 进程 × 2000 条 | ~1.0 s | InnoDB ~1.1 s | 同一随机序列文件，总 16000 次更新 |
| 并发自增 4 进程 × 250 行 | 1000 行 | — | id 1..1000 连续**无重复无缺口** |

要点：

- 高碰撞下 InnoDB 的差异来自行锁排队；FASTMEM 无表锁 + 读者无锁，
  写者行级自旋 → 耗时几乎与碰撞强度无关；
- 修复过的读-算-写路径（3.1 节）保证并发 UPDATE 不丢更新，SUM 增量
  与语句数精确相等；
- 端到端延迟仍受 MySQL 协议/连接开销限制（本地单连接净开销 ~200ms），
  引擎内部数据路径为亚微秒级。
